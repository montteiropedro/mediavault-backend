...
RSpec.describe MediaAudioCache do
  ...
  describe "#exist?" do
    ...
    it "returns true when the track exists" do
      FileUtils.touch(@tmp_dir.join("tmp", "audios", "123", "audio_track_1.m4a"))
  ...
  describe "#path" do
    it "returns the expected audio path" do
      expect(cache.path(1)).to eq(@tmp_dir.join("tmp", "audios", "123", "audio_track_1.m4a"))
    end
  end
  ...
  describe "#prepare!" do
    it "creates the cache directory" do
      expect { cache.prepare! }
        .to change { File.directory?(@tmp_dir.join("tmp", "audios", "123")) }
        .from(false)
        .to(true)
    end
  end
